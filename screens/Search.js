  ...
  componentDidMount = async () => {
    ...
    fetchMoreTransaction=async text=>{
      ...
      if(enteredText[0]=== "B"){
       ...
       const quary=await db.collection("transactions").where("bookId","==",text).startAfter(lastVisiableTransaction).limit(10).get()
       quary.docs.map(doc=>{
         this.setState({
           allTransations:[this.state.allTransactions,doc.data()],
           lastVisiableTransaction:doc
         })
       })
      }
      else  if(enteredText[0]=== "S"){
       
        const quary=await db.collection("transactions").where("StudentId","==",text).startAfter(lastVisiableTransaction).limit(10).get()
        quary.docs.map(doc=>{
          this.setState({
            allTransations:[this.state.allTransactions,doc.data()],
            lastVisiableTransaction:doc
